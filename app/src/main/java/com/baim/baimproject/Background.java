...
/**
 * Created by Ikhsan on 26/07/2016.
 */
public class Background {
    ...
    public Background(@NonNull View target, int[] drawables) {
        this.target = target;
    ...
    private void animate(final int firstDrawable, int secondDrawable, final int duration){
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                animate(localSecondDrawable, localSecondDrawable + 1, radInt(MIN,MAX));
            }
        },duration);
    }
    public void start(){
        final int duration = radInt(MIN,MAX);
        animate(0,1,duration);
    }
    public void stop(){
        handler.removeCallbacksAndMessages(null);
    }
    private int radInt(int min, int max) {
